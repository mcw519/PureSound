...
### 2026-08-18 — 收尾：抽出 `NoiseStage`

驗收：`ruff` 全綠；`--suite full` **900 passed**（+11）；現役指紋 **1954 個 hash 全同**；
6 個反向驗證中 5 個一次抓到、1 個補測試後也抓到。

「下一步」第 3 條說剩下的槓桿是把 noise 那段抽出去，但**警告「與 room_scene 耦合，切線
不像裝置鏈那麼乾淨」——那個警告是我自己下得太保守**。實際看下去，`room_scene` 是一個
**傳進來的參數**，不是阻止抽離的耦合；closure 捕捉一個參數完全沒問題。真正的邊界是：

| | |
|---|---|
| config | `augmentation_noise_args` |
| 協作者 | `self.augmentor`、`sample_rate` |
| 每列輸入 | `noisy_speech`、`room_scene` |
| 回傳 | 混音 + 實際抽到的 SNR |

跟 `DeviceChain` / `OverlapGating` 同一個形狀。抽成 `puresound/task/noise_stage.py`（171 行）：
`ns.py` **848 → 740 行**，`__getitem__` 複雜度 **25 → 18**、少 108 行。

三個噪音源之所以是三個，是因為它們對不同的東西負責：錄音噪音（相對 SNR，可選擇用**同一個
房間**的通道上色，否則噪音乾乾地來、模型免費知道哪個是語音）、白噪音（錄音池不總是給的
寬頻覆蓋）、以及**絕對位準的擷取底噪**（麥克風自噪與房間底噪不隨誰在說話而變，SNR 相對的
源表達不出來）。

**反向驗證漏一條、而且漏得有代表性**：把回傳的 SNR 從「錄音噪音那個」改成「白噪音那個」，
測試沒紅——因為 seed 0 剛好走了 dynamic-noise 分支，那條分支**根本不跑白噪音**。改成跑
6 個 seed（實測 8/10 個 seed 會進白噪音分支）之後就抓到了。這條 mutation 對應的是舊碼真實
存在的陷阱：白噪音的 SNR 本來就寫進同一個區域變數，在那之後讀就會給整列貼錯標籤。

### 計畫狀態

**P0（0-1…0-5）、P1（1-1…1-6）、P2（2-1…2-6）、P3（3-1、3-3、3-4、3-5）全部完成**；
3-2 因 A6 提前為 2-5，也已完成。加上這一節，「下一步」列的三項也清空。

`puresound/` 仍有 50 個函式複雜度 > 10，但**絕大多數在 `audio/rir/physics/`**——那是數值
演算法（`simulate_fdtd_reference` 是 72），review 從來沒有把它們列為目標，也不該用同一把尺。
task 層剩下的是 `_build_synthetic_interferers`(14)、`ns.__getitem__`(18)、
`voice_isolation.__getitem__`(19)、`tse.__getitem__`(22)、`sv.get_enroll_speech`(12)：
都是「這一列是什麼」的決策鏈，沒有像裝置鏈 / gating / noise 那樣自足的次系統可以切。
要再往下就是重新設計 row-plan 的表達方式，那是新設計而不是重構。


### 下一步

計畫已全部走完，見上面的「計畫狀態」。往後的工作是新的觀察，不是這份計畫的殘項——
真的要再壓 `task/` 的複雜度，得先重新設計 row plan 的表達方式，那屬於新設計。
