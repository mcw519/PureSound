...
English version: `impedance_measurements.md`
...
`puresound/audio/` 的舊扁平模組已在套件遷移中移除（見
[`rir_package_migration.md`](rir_package_migration.md)）；下表是遷移後的現行路徑，
每一列都已用 `.venv/bin/python -c "import ..."` 與對應 test 逐一確認可匯入。

| 檔案 | 職責 |
|------|------|
| `puresound/audio/rir/physics/impedance/measurements.py` | SI 與 normalized CSV/JSON 量測契約（`ComplexImpedanceMeasurement`、`NormalizedComplexImpedanceMeasurement`） |
| `puresound/audio/rir/physics/impedance/tube.py` | H12、換麥校正、管徑／間距 gate、coherence 與 uncertainty reduction（`TwoMicrophoneTubeGeometry`、`reduce_two_microphone_repeats`） |
| `puresound/audio/rir/physics/impedance/admittance.py` | 被動 relaxation、multi-pole 與 series-RLC admittance，以及 digital boundary reflection filter（`FirstOrderRelaxationAdmittance`、`PassiveMultiPoleAdmittance`、`PassiveResonantAdmittance`） |
| `puresound/audio/rir/physics/impedance/fitting.py` | 固定實 pole 多極點 fitting 與共軛 RLC pole fitting（`fit_passive_multi_pole_admittance()`、`fit_passive_single_resonance_admittance()`） |
| `puresound/audio/rir/physics/wave/fdtd.py` | 每牆面 cell 的一階／biquad boundary state 3D FDTD reference solver（`FDTDReferenceConfig`、`simulate_fdtd_reference()`） |
| `puresound/audio/rir/physics/impedance/modes.py` | 1D cavity 與 separable 3D 六面 rational impedance eigenproblem（`solve_1d_impedance_cavity_modes()`、`solve_rectangular_impedance_modes()`） |
| `puresound/audio/rir/physics/impedance/residues.py` | fixed-pole complex residue fitting、position holdout 與版本化 calibration（`fit_fixed_pole_modal_residues()`、`ImpedanceModalResidueCalibration`） |
| `puresound/audio/rir/render/low_frequency/impedance_modal.py` | experimental `ImpedanceModalLowFrequencyBackend`：separable 3D rational-impedance modal renderer，於 R2（`RIR_EXP_LOG.md`）由 `hybrid_rir` 移出獨立成檔 |
| `puresound/audio/rir/render/hybrid.py` | `generate_hybrid_rir()`：組裝低頻／高頻 backend、causality clip 與 crossover 成完整 hybrid RIR 的 orchestration 入口；即舊 `hybrid_rir.py` 的直接後繼 |
...
| `test/test_acoustic_impedance.py` | admittance 模型 passivity 與 digital boundary filter 測試 |
...
| `test/test_impedance_priors.py` | Miki prior fitting 與 FDTD modal diagnostic 測試 |
| `test/test_fdtd_reference.py` | FDTD boundary state 與 1D plane-wave reflection 測試 |
| `test/test_impedance_residues.py` | fixed-pole residue fitting 與 split-holdout gate 測試 |
| `test/test_impedance_residue_protocol.py` | M2.9 residue protocol CLI 測試 |
| `test/test_impedance_validation_cli.py` | impedance validation CLI smoke 測試 |
