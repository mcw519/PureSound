...
[`rir_realism_algorithm_zh-TW.md`](rir_realism_algorithm_zh-TW.md) §7.
...
| Spatial (synchronized receivers only) | `analyze_binaural_iacc`, `analyze_array_spatial_coherence`, `diffuse_field_coherence` |
...
- A decay estimate is only as good as its fit: check `DecayEstimate` R² (M6 QC
  uses ≥ 0.70) before trusting the seconds value.
- `estimate_noise_floor_lundeby` needs ≥ 15 dB of dynamic range; corpora with
  faded tails report "no floor" for reasons unrelated to measurement noise.
...
```python
from puresound.audio.rir.metrics import analyze_rir

report = analyze_rir(rir_channel, 16000, echo_density=True)
