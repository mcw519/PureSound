...
> 它挖出的四個新問題寫在 [§4](#4-ab-pilot-的結果與由它挖出的問題)，其中一個是
> **ISO 9613-1 大氣吸收的濕度單位錯了**（§4.3b，4 kHz 少 5 倍）。一句話：
...
### 4.3b 順帶查出一個 physics bug：大氣吸收的濕度單位錯了
...
1. **本身就是約 1 dB/oct 的 tilt 誤差。** 4 kHz 少了 0.024 dB/m，在 0.5 s 殘響對應
   的 171 m 路徑上就是 **4.1 dB**；攤在 250 Hz–4 kHz 四個八度約 1 dB/oct。這是
   §4.1 那個殘留約 2.2 dB/oct 缺口的最大單一已知來源。
2. **空氣吸收正是現實中防止 RT60 隨頻率上升的機制。** 改成 ISO 後反向率
   32.9% → **26.7%**（不是主因，但真實）。

**未修**：這會改變之後每一個 RIR 的輸出，影響既有 bank 的可比性，該由你決定何時
動。修法是把那個 `/ 100.0` 拿掉，並補一個對照 ISO 公開表值的測試。
