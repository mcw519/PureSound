...
> - **我在 §4.1 記的「tilt 符號錯」已撤回**（§4.4）。那個實測參照不是房間物理，是五
>   條互相矛盾的量測鏈的加權平均——語料中位數相距 3.68 dB/oct，其中 ACE 幾乎等於
>   合成值。**先驗證目標再追它，救回了一整條錯誤的工作方向。**
...
**兩者共有、換後端不會改善的**：~~tilt 符號錯~~（**撤回，§4.4**）、完全沒有噪音地板
