...
class DemoTseNet(nn.Module):
    ...
    def streaming_inference(
        self, chunk: torch.Tensor, embed: torch.Tensor
    ) -> torch.Tensor:
        if embed.dim() == 1:
    ...
    @torch.no_grad()
    def streaming_inference_chunk(
        self,
        chunk: torch.Tensor,
        embed: torch.Tensor,
        pre_wav: Optional[torch.Tensor] = None,
    ) -> torch.Tensor:
        if embed.dim() == 1:
            embed = embed.unsqueeze(0)
