...
class Parser:
    ...
    @staticmethod
    def read_librispeech_metadata(
        f_path: str, insert_root: Optional[str] = None
    ) -> Dict:
        """
        Args:
        ...
        with io.open(f_path, "r", encoding="utf-8") as f:
            for idx, line in enumerate(f.readlines()):
                ...
                meta_dict[spkid]["utts"].update(
                    {uttid: {"path": audio_path, "length": length}}
                )
...
if __name__ == "__main__":
    ...
    meta_pool = Parser.read_librispeech_metadata(
        f_path=config.corpus_metadata, insert_root=config.corpus_root
    )
    dct = {}
    ...
    for key in sorted(dct.keys()):
        with io.open(
            f"{config.output_folder}/wav2scp.txt", "a+", encoding="utf-8"
        ) as f:
            f.writelines(f"{key} {dct[key]['noisy']}\n")

    for key in sorted(dct.keys()):
        with io.open(
            f"{config.output_folder}/wav2ref.txt", "a+", encoding="utf-8"
        ) as f:
            f.writelines(f"{key} {dct[key]['ref']}\n")

    for key in sorted(dct.keys()):
        with io.open(
            f"{config.output_folder}/ref2spk.txt", "a+", encoding="utf-8"
        ) as f:
            f.writelines(f"{key} {dct[key]['spk']}\n")

    for key in sorted(dct.keys()):
        with io.open(
            f"{config.output_folder}/wav2spk.txt", "a+", encoding="utf-8"
        ) as f:
            f.writelines(f"{key} {dct[key]['all_spks']}\n")

    for key in sorted(dct.keys()):
        with io.open(
            f"{config.output_folder}/ref2list.txt", "a+", encoding="utf-8"
        ) as f:
