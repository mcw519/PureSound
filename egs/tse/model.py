...
def init_loss(hparam):
    ...
    if sig_loss.lower() in ["sisnr", "sdsdr", "sdr", "tsdr"]:
        ...
        sig_loss = lambda enh, ref, others: stft_loss(enh, ref) + sdr_loss(
            enh, ref, others
        )

    elif sig_loss.lower() == "sisnr_ov":
        sdr_loss = SDRLoss.init_mode("sisnr", threshold=sig_threshold)
        sig_loss = lambda enh, ref, others: sdr_loss(
            enh, ref, others
        ) + over_suppression_loss(enh, ref)
    ...
    if cls_loss.lower() == "cross_entropy":
        ...
        cls_loss = AAMsoftmax(
            input_dim=hparam["LOSS"]["embed_dim"],
            n_class=hparam["LOSS"]["n_class"],
            margin=hparam["LOSS"]["margin"],
            scale=hparam["LOSS"]["scale"],
        )

    elif cls_loss.lower() == "ge2e":
        assert hparam["TRAIN"]["contrastive_learning"]
        cls_loss = GE2ELoss(
            nspks=hparam["TRAIN"]["p_spks"],
            putts=hparam["TRAIN"]["p_utts"],
            add_norm=True,
        )
...
def init_model(
    name: str,
    sig_loss: Optional[nn.Module] = None,
    ...
    **kwargs
    ...
    if name == "td_tse_conv_tasnet_v0":
        ...
        model = SoTaskWrapModule(
            ...
            masker=ConvTasNet(
                ...
                tcn_dilated_basic=2,
                per_tcn_stack=8,
                tcn_with_embed=[1, 0, 0, 0, 0, 0, 0, 0],
                tcn_norm="gLN",
                dconv_norm="gGN",
                causal=False,
                tcn_layer="normal",
            ...
            speaker_net=nn.ModuleList(
                ...
                + [
                    AttentiveStatisticsPooling(512, 128),
                    nn.Conv1d(512 * 2, 192, 1, bias=False),
                ]
            ),
        ...
        model = SoTaskWrapModule(
            ...
            masker=UnetTcn(
                ...
                skip_conv=False,
                kernel_t=(2, 2, 2, 2, 2, 2),
                kernel_f=(5, 5, 5, 5, 5, 5),
                stride_t=(1, 1, 1, 1, 1, 1),
                stride_f=(2, 2, 2, 2, 2, 2),
                dilation_t=(1, 1, 1, 1, 1, 1),
                dilation_f=(1, 1, 1, 1, 1, 1),
                delay=(0, 0, 0, 0, 0, 0),
                tcn_layer="gated",
                tcn_kernel=3,
                tcn_dim=256,
            ...
            speaker_net=nn.ModuleList(
                ...
                + [
                    GatedTCN(256, 128, 3, dilation=2 ** i, causal=False, tcn_norm="gLN")
                    for i in range(5)
                ]
                + [
                    AttentiveStatisticsPooling(256, 128),
                    nn.Conv1d(256 * 2, 192, 1, bias=False),
                ]
            ),
            loss_func_wav=sig_loss,
            loss_func_spk=cls_loss,
        ...
        model = SoTaskWrapModule(
            ...
            speaker_net=nn.ModuleList(
                ...
                + [
                    AttentiveStatisticsPooling(128, 128),
                    nn.Conv1d(128 * 2, 192, 1, bias=False),
                ]
            ),
            loss_func_wav=sig_loss,
            loss_func_spk=cls_loss,
    ...
    elif name == "tse_skim_v2_causal":
        """
        Total params: 6,221,554
        Lookahead(samples): 16
        Receptive Fields(samples): infinite
        """
        model = SoTaskWrapModule(
            ...
            encoder_spk=FbankEnc(
                trainable=False, output_format="Magnitude", n_banks=80
            ),
            masker=SkiM(
                input_size=128,
                hidden_size=256,
            ...
            speaker_net=nn.ModuleList(
                [SpecAugment(freq_mask_length=10, time_mask_length=0, fill_value=0.0)]
                + [
                    TCN(
                        80,
                        256,
                ...
                + [
                    AttentiveStatisticsPooling(80, 128),
                    nn.Conv1d(80 * 2, 192, 1, bias=False),
                ]
            ),
            loss_func_wav=sig_loss,
            loss_func_spk=cls_loss,
