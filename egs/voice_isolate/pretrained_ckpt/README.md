...
|---|---|---|---|---|
| `dpcrn_v1.ckpt` | internal | cold | curriculum core: RT60 0.20–0.45, near/far DRR gap ≥ 6 dB | in-domain SI-SDRi median **+6.98** (ep39) |
| `dpcrn_v2.ckpt` | internal | v1 | widened curriculum: RT60 ≤ 0.65, DRR gap ≥ 3 dB | in-domain **+7.99**; measured-RIR set +5.46; first real-WER win (BUT enh 0.777 < mix 0.796) (ep59) |
| `dpcrn_v3.ckpt` | internal | v2 | `OverSuppressionLoss` weight 1.0 (anti-deletion) | in-domain **+8.21**; BUT deletion 0.308 → 0.291 (ep19) |
| `dpcrn_v4.ckpt` | internal | v3 | anti-suppression weight 2.0 | in-domain **+8.32**; BUT deletion → 0.276; most even across domains (ep19) |
| `dpcrn_v5.ckpt` | internal | v4 | anti-suppression weight 3.0 | in-domain **+8.45**; best in deployment-level reverb, worst in extreme reverb — the domain split point (ep19) |
| `dpcrn_v6.ckpt` | `test/fixtures/recipes/train_dpcrn_wide_antisup.yaml` | v5 | wide RIR domain (RT60 0.20–0.85) + capture realism (media-voice interferer, HPF) | held-out unseen-room **+8.06**; **first streaming-verified** version (see below) (ep19) |
| `dpcrn_v7.ckpt` | internal | v6 | real recordings on both sides of the decision (real far interferers + real <1 m keep rows), turn-taking far-solo supervision, distance/DRR aux head, channel-perturbation mask consistency | held-out real far-field **−9.45 dB, graded by distance** (1–2 m −3 → 5 m+ −38; v6: −1.76 flat), near-field keep flat (−0.11), **Dawn WER 0.174 < 0.184 raw**, deletion 0.088 ≈ raw floor, reverberant-office WER −0.024 vs mix, in-domain +8.18 (ep19, with `dry_blend 0.9`) |
...
judging against them, so restore a file from git history when you need one
...
`dpcrn_v6_gate.ckpt` — off the main line: internal freezes v6 and
