...
def scan_checkpoints(config_path: str | Path) -> list[tuple[str, str]]:
    ...
    search_roots.append(recipe_root / "exp")
    LOGGER.info(
        "Scanning checkpoints from config=%s roots=%s",
        config_path,
...
def refresh_checkpoints(config_path: str | Path):
    try:
        choices = scan_checkpoints(config_path)
    ...
    if not choices:
        return (
            ...
            "No checkpoints found. Train a model or place .ckpt/.pt/.pth files under work_folder or ./exp.",
        )

    return gr.update(choices=choices, value=choices[0][1]), f"Found {len(choices)} checkpoint(s)."
