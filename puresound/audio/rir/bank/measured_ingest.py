...
def prune_bank_to_qc_passed(
    ...
    *,
    ...
    A release variant has to hold only passing items — M6's production decision
    checks every item in every variant manifest — but a measured bank always
    carries some quarantine, because some published rooms genuinely fail the
    acoustic gates and forcing them through is the one thing this pipeline must
    not do. So the release takes a pruned copy, and the unpruned bank stays as
    the record of what was dropped and why.

    Renderer profiles are kept even when every one of their items was dropped, so
    a corpus that contributed nothing is still visible in the manifest rather
