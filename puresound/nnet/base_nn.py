...
class EncDecMaskerBaseModel(BaseModel):
    ...
    def apply_tf_masks(
        self, tf_rep: torch.Tensor, est_masks: torch.Tensor, mask_type: str, f_type: str
    ) -> torch.Tensor:
        """
    ...
    def get_mask(
        self, mask: torch.Tensor, mask_constraint: str = "linear"
    ) -> torch.Tensor:
    ...
    def _apply_complex_mask_on_reim(
        self, tf_rep: torch.Tensor, est_masks: torch.Tensor
    ) -> torch.Tensor:
        """
    ...
    def _apply_complex_mask_on_polar(
        self, tf_rep: torch.Tensor, est_mask: torch.Tensor
    ) -> torch.Tensor:
        """
...
class SoTaskWrapModule(EncDecMaskerBaseModel):
    ...
    def check_task(self):
        ...
        if self.speaker_net is None:
            ...
            if self.encoder_spk is not None:
                task_label = 1
                assert (
                    self.speaker_net is not None
                ), f"Multi-task trainer missed the SpeakerNet."
                print(
                    f"Initialized a multi-task model, including two separate speech encoder."
                )
    ...
    def _get_feature(
        ...
        enroll: Optional[torch.Tensor] = None,
    ...
    def _get_waveform(self, enh_feats: torch.Tensor) -> torch.Tensor:
        if isinstance(self.encoder, ConvEncDec):
            ...
            if self.drop_first_bin:
                ...
                _im = enh_feats[..., 1]
                pad = torch.zeros(
                    _re.shape[0], 1, _re.shape[2], device=enh_feats.device
                )
    ...
    def _forward(
        ...
        enh_feats = self.apply_tf_masks(
            noisy, mask, f_type=self.f_type, mask_type=self.mask_type
        )  # [N, C, T]
    ...
    def _forward_join_loop(
        ...
        triplet_dvec = torch.stack(
            [dvec.squeeze(-1), enh_dvec.squeeze(-1), noise_dvec.squeeze(-1)], dim=1
        )
        ...
        if self.loss_func_spk is not None and spk_class is not None:
            ...
            if return_loss_detail:
                return (
                    loss_wav + alpha * loss_spk + (1 / alpha) * loss_spk_loop,
                    (loss_wav, loss_spk, (1 / alpha) * loss_spk_loop),
                )
    ...
    @torch.no_grad()
    def inference(
        self, noisy: torch.Tensor, enroll: Optional[torch.Tensor] = None
    ) -> torch.Tensor:
        noisy, enroll = self._get_feature(noisy, enroll)  # [N, C, T]
        dvec = enroll
    ...
    @torch.no_grad()
    def inference_tse_embedding(
        self, enroll: Optional[torch.Tensor] = None
    ) -> torch.Tensor:
...
class SiMoTaskWrapModule(EncDecMaskerBaseModel):
    """
    Model wrapper for a Single Input Multi Output(SIMO).
    Like speech separation task.
    ...
    Main structure has:
        Encoder -> Masker -> Decoder(inside encoder's inverse). // This is `Speech Separation` Task
    ...
    def _get_feature(self, noisy: Optional[torch.Tensor] = None) -> torch.Tensor:
        """
        Forward encoder to get features.
    ...
    def _get_waveform(self, enh_feats: torch.Tensor) -> torch.Tensor:
        assert (
            enh_feats.dim() == 4
        ), "SIMO task needed 4D tensor has shape [N, Ch, C, T]"

        batch, chout, fdim, tdim = enh_feats.shape
        enh_feats = enh_feats.reshape(-1, fdim, tdim)
        ...
        enh_wav = self.encoder.inverse(enh_feats)
        enh_wav = enh_wav.reshape(batch, chout, -1)
        return enh_wav
    ...
    def forward(
        ...
        Args:
            noisy: Input noisy mixture tensor, [N, T]
            ref_clean: Target speech, [N, M, T]
            inactive_labels: Inactive speaker labels, [N, M]

        Return:
            Loss: loss_sdr
        """
        noisy = self._get_feature(noisy)  # [N, C, T]
        mask = self.masker(noisy)  # [N, M, C, T]
        mask = self.get_mask(mask, self.mask_constraint)

        batch, chout, fdim, tdim = mask.shape
        mask = mask.reshape(-1, fdim, tdim)
        noisy = noisy.unsqueeze(1).repeat(1, chout, 1, 1)
        noisy = noisy.reshape(-1, fdim, tdim)
        enh_feats = self.apply_tf_masks(
            noisy, mask, f_type=self.f_type, mask_type=self.mask_type
        )  # [N, C, T]
        enh_feats = enh_feats.reshape(batch, chout, fdim, tdim)  # [N, M, C, T]
        enh_wav = self._get_waveform(enh_feats)  # [N, M, T]
        enh_wav = self._wav_output_constrain(enh_wav, mode=self.output_constraint)
        enh_wav, ref_clean = self._align_waveform(enh_wav, ref_clean)
        enh_wav = enh_wav.reshape(batch * chout, -1)
        ref_clean = ref_clean.reshape(batch * chout, -1)
        loss_wav = self.loss_func_wav(enh_wav, ref_clean, inactive_labels.reshape(-1))

        return loss_wav

    @torch.no_grad()
    def inference(self, noisy: torch.Tensor) -> torch.Tensor:
        noisy = self._get_feature(noisy)  # [N, C, T]
        mask = self.masker(noisy)  # [N, M, C, T]
        mask = self.get_mask(mask, self.mask_constraint)
