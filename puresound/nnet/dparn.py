...
class DPARNblock2D(nn.Module):
    """
    DPARN-2D modul which be parts of DPARN, which replaced the intra-LSTM by Self-Attention.

    Args:
        input_size: input 4D-tensor's channel dimension
        hidden_size: RNN's hidden dimension
        nhead: number of heads in MHA
    ...
    def __init__(self,
        ...
        self.intra_atten1 = MhaSelfAttenLayer(input_size, hidden_size, nhead=nhead, dropout=dropout, improved=False, bidirectional=False, position_encoding=True) # already has a LayerNorm inside
        self.intra_atten2 = MhaSelfAttenLayer(input_size, hidden_size, nhead=nhead, dropout=dropout, improved=False, bidirectional=False, position_encoding=False) # already has a LayerNorm inside
    ...
    def forward(self, x: torch.Tensor, intra_skip: bool = True, inter_skip: bool = True) -> torch.Tensor:
        ...
        x = self.intra_atten1(x.permute(0, 2, 1), causal=False) # [N*T, C, CH] -> [N*T, CH, C]
        x = x.permute(0, 2, 1) # [N*T, C, CH]
        x = self.intra_atten2(x.permute(0, 2, 1), causal=False) # [N*T, C, CH] -> [N*T, CH, C]        
        x = x.permute(0, 2, 1) # [N*T, C, CH]
        x = self.intra_fc(x)
        x = self.intra_norm(x)
        x = x.reshape(N, T, C, -1)
...
class DPARN(Unet):
    def __init__(self,
        ...
        # DPRNN block
        self.dprnn_block1 = DPARNblock2D(input_size=channels[-1], hidden_size=rnn_hidden, nhead=nhead, dropout=dropout)
        self.dprnn_block2 = DPARNblock2D(input_size=channels[-1], hidden_size=rnn_hidden, nhead=nhead, dropout=dropout)
