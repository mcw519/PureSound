...
class DPRNN(nn.Module):
    ...
    def __init__(self,
        ...
        for i in range(n_blocks):
            if embed_dim != 0 and block_with_embed[i]:
                self.intra_rnn.append(nn.LSTM(input_size, hidden_size, num_layers=1, bidirectional=self.bi_direct, batch_first=True))
                self.input_film.append(FiLM(input_size, embed_dim, input_norm=True))
            else:
                self.intra_rnn.append(nn.LSTM(input_size, hidden_size, num_layers=1, bidirectional=self.bi_direct, batch_first=True))
                self.input_film.append(None)
            
            self.intra_proj.append(nn.Linear(hidden_size*(int(self.bi_direct)+1), input_size))
            self.intra_norm.append(nn.LayerNorm(input_size))
            self.inter_rnn.append(nn.LSTM(input_size, hidden_size, num_layers=1, bidirectional=self.bi_direct, batch_first=True))
            self.inter_proj.append(nn.Linear(hidden_size*(int(self.bi_direct)+1), input_size))
    ...
    def forward(self, x: torch.Tensor, embed: Optional[torch.Tensor] = None):
        ...
        if self.seg_overlap:
            x, rest = SplitMerge.split(x, seg_size=self.seg_size) # [N, C, T] -> [N, S, K, C]
        ...
        for i in range(self.n_blocks):
            ...
            inter_output, _ = self.inter_rnn[i](inter_input)
            ...
            output = output.reshape(batch_size, seg_size, total_seg, feat_size).contiguous()
