...
class MHA(nn.Module):
    ...
    def forward(self, query: torch.Tensor, key: torch.Tensor, value: torch.Tensor, causal: bool = True, context_range: int = None):
        """
        Args:
        ...
        if causal and context_range is None:
            mask = (torch.triu(torch.ones(size_q, size_q)) == 1).transpose(0, 1) # [L, S]
        ...
        if context_range is not None:
            if causal:
                ...
                mask = torch.tril(torch.ones(size_q, size_q), diagonal=-(context_range-1))
                mask = mask + mask.transpose(0, 1)
                mask = mask.float().masked_fill(mask == 0, float(0.0)).masked_fill(mask == 1, float('-inf'))
        
        if mask is not None: mask = mask.to(query.device)
...
class MhaSelfAttenLayer(nn.Module):
    ...
    def __init__(self,
        ...
        if not improved:
            ...
            if position_encoding:
                print("Ignored position_encoding option here replaced by LSTM modeling.")

            self.recurrent = nn.LSTM(feats_dim, hidden_dim, bidirectional=bidirectional, batch_first=True)
    ...
    def forward(self,
        ...
        """
        if self.improved: self.recurrent.flatten_parameters()
