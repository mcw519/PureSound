...
class Gate(nn.Module):
    def __init__(
        self, input_size: int, hidden_size: int, embed_size: int, dropout: float = 0.0
    ):
        super().__init__()
        self.in_conv = nn.Conv1d(
            input_size, hidden_size, kernel_size=1, bias=False, groups=1
        )
        ...
        self.out_conv = nn.Conv1d(
            hidden_size, input_size, kernel_size=1, bias=False, groups=1
        )
...
class FiLM(nn.Module):
    ...
    def __init__(self, feats_size: int, embed_size: int, input_norm: bool = True):
        ...
        self.cond_scale = nn.Conv1d(
            feats_size + embed_size, feats_size, kernel_size=1, bias=False
        )
