...
class MemLSTM(nn.Module):
    def __init__(self, hidden_size: int, causal: bool = True, dropout: float = 0.0):
        super().__init__()
        ...
        self.h_proj = nn.Linear(
            self.hidden_size * (int(self.bi_direct) + 1), self.input_size
        )
    ...
    def forward(
        ...
        h_ = self.h_proj(h_.reshape(batch * total_seg, -1)).reshape(
            batch, total_seg, -1
        )
        h = h + self.h_norm(h_)
        ...
        c_ = self.c_proj(c_.reshape(batch * total_seg, -1)).reshape(
            batch, total_seg, -1
        )
        c = c + self.c_norm(c_)

        h = (
            h.reshape(batch * total_seg, direct, feat_len).transpose(1, 0).contiguous()
        )  # [D, NS, C]
        c = (
            c.reshape(batch * total_seg, direct, feat_len).transpose(1, 0).contiguous()
        )  # [D, NS, C]
    ...
    def streaming_forward(
        ...
        h = (
            torch.cat(h_out, dim=1)
            .reshape(batch * total_seg, direct, feat_len)
            .transpose(1, 0)
            .contiguous()
        )
        c = (
            torch.cat(c_out, dim=1)
            .reshape(batch * total_seg, direct, feat_len)
            .transpose(1, 0)
            .contiguous()
        )
...
class SegLSTM(nn.Module):
    ...
    def forward(self, x: torch.Tensor, h: torch.Tensor, c: torch.Tensor):
        ...
        x_out = self.proj(x_out.contiguous().view(-1, x_out.shape[2])).view(
            batch, seg_size, feat_len
        )
    ...
    def streaming_forward(self, x: torch.Tensor, h: torch.Tensor, c: torch.Tensor):
        ...
        for idx in range(x.shape[1]):
            ...
            _out = self.drop(_out)
            _out = self.proj(_out.contiguous().view(-1, _out.shape[2])).view(
                batch, 1, feat_len
            )
...
class SkiM(nn.Module):
    ...
    def split(self, x: torch.Tensor):
        ...
        seg_1 = (
            x[:, :, :-seg_stride].contiguous().view(batch, feat_size, -1, self.seg_size)
        )
        seg_2 = (
            x[:, :, seg_stride:].contiguous().view(batch, feat_size, -1, self.seg_size)
        )

        segments = torch.cat([seg_1, seg_2], dim=-1).view(
            batch, feat_size, -1, self.seg_size
        )  # [N, C, S, K]
    ...
    def merge(self, x: torch.Tensor, rest: int):
        ...
        x1 = (
            x[:, :, :, :seg_size]
            .contiguous()
            .view(batch, feat_size, -1)[:, :, seg_stride:]
        )
        x2 = (
            x[:, :, :, seg_size:]
            .contiguous()
    ...
    def forward(self, x: torch.Tensor, embed: Optional[torch.Tensor] = None):
        ...
        for i in range(self.n_blocks):
            ...
            if i < self.n_blocks - 1:
                h = h.reshape(-1, N, S, self.hidden_size).permute(
                    1, 2, 0, 3
                )  # [D, N, S, C] -> [N, S, D, C]
                c = c.reshape(-1, N, S, self.hidden_size).permute(
                    1, 2, 0, 3
                )  # [D, N, S, C] -> [N, S, D, C]
