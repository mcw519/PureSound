...
class Unet(nn.Module):
    ...
    """
    def __init__(self,
                input_type: str = 'RI', input_dim: int = 512,
                activation_type: str = 'PReLU', norm_type: str = 'bN2d', dropout: float = 0.05,
                channels: Tuple = (1, 1, 8, 8, 16, 16), transpose_t_size: int = 2, skip_conv: bool = False,
                kernel_t: Tuple = (5, 1, 9, 1, 1), stride_t: Tuple = (1, 1, 1, 1, 1), dilation_t: Tuple = (1, 1, 1, 1, 1),
                kernel_f: Tuple = (1, 5, 1, 5, 1), stride_f: Tuple = (1, 4, 1, 4, 1), dilation_f: Tuple = (1, 1, 1, 1, 1),
                delay: Tuple = (0, 0, 1, 0, 0),
                ):
        super().__init__()
        assert len(kernel_t) == len(kernel_f) == len(stride_t) == len(stride_f) == len(dilation_t) == len(dilation_f)
        ...
        self.kernel = list(zip(kernel_f, kernel_t)) # each layer's kernel size (freq, time)
        ...
        for i in range(self.n_cnn):
            encode = []
            freq_pad = (self.kernel[i][0]//2, self.kernel[i][0]//2) # center padding in frequency axis
            time_pad = (self.kernel[i][1] - self.delay[i] - 1, self.delay[i])
            encode += [
                nn.ZeroPad2d(time_pad + freq_pad), # (left, right, top, down)
                nn.Conv2d(self.channels[i], self.channels[i+1], kernel_size=self.kernel[i], stride=self.stride[i], dilation=self.dilation[i]),
        ...
        for i in reversed(range(self.n_cnn)):
            ...
            if i != 0:
                encode += [
                    nn.ConvTranspose2d(self.channels[i+1]*skip_double[i], self.channels[i], kernel_size=(k, self.t_kernel), stride=self.stride[i], dilation=self.dilation[i],
                        padding=(p, 0), output_padding=(op, 0)),
        ...
        if skip_conv:
            ...
            for i in reversed(range(self.n_cnn)):
                encode = []
                encode += [
                    nn.Conv2d(self.channels[i+1], self.channels[i+1], kernel_size=(1, 1), stride=1),
                    active_cls()
                ]
    ...
    def shape_info(self):
        ...
        for i in range(self.n_cnn):
            ...
            _f = (up_shape[-1] - 1) * stride - 2*padding + self.dilation[-i-1][0] * (kernel_size-1) + output_padding + 1
            up_shape.append(_f)
...
class UnetTcn(Unet):
    ...
    def forward(self, x: torch.Tensor, dvec: Optional[torch.Tensor] = None) -> torch.Tensor:
        """
        Args:
            x: input tensor shape [N, C, T]
