...
class AudioIO():
    ...
    @staticmethod
    def open(f_path: str, normalized: bool = False, target_lvl: Optional[float] = None, verbose: bool = False):
        """
        ...
        if normalized:
            if target_lvl is not None and verbose:
                print(f"You choosed the waveform nomalized, the target_lvl would not be used.")
                wav = AudioIO.normalize_waveform(wav=wav, amp_type='avg')
                    
        if target_lvl is not None:
            wav = AudioIO.rescale_waveform(wav=wav, target_lvl=target_lvl, amp_type='avg', scale='dB')
            avg_amp_rescale = torch.mean(torch.abs(wav), dim=-1)

        if verbose:
            print(f"Open file: {f_path}")
            print(f"Avg_amp: {avg_amp_ori}")
            if target_lvl is not None: print(f"Avg_amp_rescale: {avg_amp_rescale}")
    ...
    def audio_cut(wav: torch.Tensor, sr: int, length_s: float):
        ...
        if len(wav.shape) == 1: wav = wav.unsqueeze(0)
        wav, offset, end_offset = AudioIO.cut_audio(wav=wav, sr=sr, length_s=length_s, padding=True)
    ...
    @staticmethod
    def rescale_waveform(wav: torch.Tensor, target_lvl: float, amp_type: str = 'avg', scale: str = 'linear') -> torch.Tensor:
        """
    ...
    @staticmethod
    def cut_audio(wav: torch.Tensor, sr: int, length_s: int, padding: bool = False) -> Tuple[torch.Tensor, int, int]:
        """
        Random cut audio in specific length, if not enough length padding zeros after sequences.
...
class AudioAugmentor():
    ...
    def sox_volumn_perturbed(self, wav: torch.Tensor, vol_ratio: float, sr: int = 16000) -> torch.Tensor:
        """
    ...
    def sox_speed_perturbed(self, wav: torch.Tensor, speed: float, sr: int = 16000) -> Tuple[torch.Tensor, float]:
        """
    ...
    def add_bg_noise(self, wav: torch.Tensor, snr_list: List) -> List:
        ...
        if noise.shape[0] != 1: noise = noise[0, :].view(1, -1)
        if sr != self.sr: noise = torchaudio.transforms.Resample(orig_freq=sr, new_freq=self.sr)(noise)
    ...
    def apply_rir(self, wav: torch.Tensor) -> Tuple[torch.Tensor, str, Optional[int]]:
        ...
        if sr != self.sr:
            impaulse = torchaudio.transforms.Resample(orig_freq=sr, new_freq=self.sr)(impaulse)
    ...
    def apply_rir_by_key(self, wav: torch.Tensor, key: str, choose_ch: int = None, rir_mode: str = 'image', sr: int = 16000) -> torch.Tensor:
        """
