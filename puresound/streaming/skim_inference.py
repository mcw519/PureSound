...
class StreamingSkiM(SkiM):
    ...
    def init_status(self):
        ...
        self.seg_lstm_h_states = [torch.zeros(D, 1, self.hidden_size) for i in range(self.n_blocks)]
        self.seg_lstm_c_states = [torch.zeros(D, 1, self.hidden_size) for i in range(self.n_blocks)]

        # init Mem-LSTM's status
        self.mem_lstm_h_hidden = [(torch.zeros(D, 1, self.hidden_size), torch.zeros(D, 1, self.hidden_size)) for i in range(self.n_blocks - 1)]
        self.mem_lstm_c_hidden = [(torch.zeros(D, 1, self.hidden_size), torch.zeros(D, 1, self.hidden_size)) for i in range(self.n_blocks - 1)]
        print(f"{time.asctime(time.localtime(time.time()))}, Initialized streaming SkiM model")
    ...
    def step_frame(self, x: torch.Tensor, embed: torch.Tensor):
        ...
        for i in range(self.n_blocks):
            if embed is not None and self.block_with_embed[i]:
                x = self.seg_input_fusion[i](x.view(1, -1, 1), embed) # fusion layer need [N, C, T]
                x = x.reshape(1, 1, -1)

            x, seg_h, seg_c = self.seg_lstm[i](x, self.seg_lstm_h_states[i], self.seg_lstm_c_states[i]) # cur_inp=[1, 1, C], seg_h/seg_c=[D, 1, C]
    ...
    def update_mem_lstm(self):
        ...
        # inter-segment-op
        for i in range(self.n_blocks-1):
            seg_h = cur_seg_lstm_h_states[i].reshape(-1, 1, 1, self.hidden_size).permute(1, 2, 0, 3)
            seg_c = cur_seg_lstm_c_states[i].reshape(-1, 1, 1, self.hidden_size).permute(1, 2, 0, 3)
            mem_h, mem_c, _hid_hidden, _cell_hidden = self.mem_lstm[i](seg_h, seg_c, h_states=self.mem_lstm_h_hidden[i],
                c_states=self.mem_lstm_c_hidden[i], return_all=True, streaming=True) # [N, S, D, C]
