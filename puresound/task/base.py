    ...
    def __init__(self, type: str, optimizer: Any, **kwargs):
        ...
        if type == "stepLR":
            self.scheduler = optim.lr_scheduler.StepLR(
                optimizer, step_size=wait, gamma=gamma
            )

        elif type == "Plateau":
            mode = "min" if not kwargs else kwargs["mode"]
            self.scheduler = optim.lr_scheduler.ReduceLROnPlateau(
                optimizer, mode=mode, factor=gamma, patience=wait
            )
...
class BaseTrainer:
    ...
    def __init__(self, hparam: Dict, device_backend: str = "cuda"):
        ...
        if self.hparam["TRAIN"]["use_tensorboard"]:
            ...
            self.tf_writer = TensorboardWriter(
                logging_path=self.hparam["TRAIN"]["log_dir"]
            )
    ...
    def build_optim(self, custom_params_group: Optional[List] = None) -> None:
        ...
        if self.hparam["TRAIN"]["resume_epoch"]:
            ...
            _, lr, _ = self.load_ckpt(
                f"{self.hparam['TRAIN']['model_save_dir']}/epoch_{self.hparam['TRAIN']['resume_epoch'] -1}.ckpt",
                self.model,
            )
            self.lr = lr
        ...
        if custom_params_group is None:
            self.optimizer = torch.optim.Adam(
                self.model.parameters(), lr, [beta1, beta2], weight_decay=weight_decay
            )
            ...
            self.optimizer = torch.optim.Adam(
                total_params_group, lr, [beta1, beta2], weight_decay=weight_decay
            )

        self.scheduler = LearningRateScheduler(
            self.hparam["OPTIMIZER"]["lr_scheduler"],
            self.optimizer,
            gamma=self.hparam["OPTIMIZER"]["gamma"],
            patience=self.hparam["OPTIMIZER"]["patience"],
            mode=self.hparam["OPTIMIZER"]["mode"],
        )

    def save_ckpt(
        self, filename: str, model: Any, epoch: int, learning_rate: Any, loss: Any
    ) -> None:
    ...
    def save_ckpt_info(
        self, filename: str, epoch: int, learning_rate: Any, loss: Any
    ) -> None:
    ...
    def train(self):
        ...
        start_epoch = (
            0
            if self.hparam["TRAIN"]["resume_epoch"] == None
            else self.hparam["TRAIN"]["resume_epoch"]
        )
        ...
        for epoch in range(start_epoch, self.num_epochs):
            ...
            if self.tf_writer:
                ...
                self.tf_writer.update_ep_loss(
                    "train/avg_loss", loss["total_loss"], epoch
                )
                self.tf_writer.update_ep_loss(
                    "train/avg_dev_loss", dev_loss["total_loss"], epoch
                )

            model_path = os.path.join(
                self.hparam["TRAIN"]["model_save_dir"], f"epoch_{epoch}.ckpt"
            )
            info_path = os.path.join(
                self.hparam["TRAIN"]["model_save_dir"], f"epoch_{epoch}.info"
            )
