...
def test_mem_lstm():
    ...
    with torch.no_grad():
        ...
        for i in range(10):
            ...
            h2, c2, h2_hid, c2_hid = memlstm.streaming_forward(
                h2, c2, h2_hid, c2_hid, return_all=True
            )
            h2_out.append(h2)
            c2_out.append(c2)
    ...
    assert (
        torch.mean((h1 - h2).abs()) < 1e-6
    ), f"mean abs error: {torch.mean((h1 - h2).abs())}, max_error: {(h1 - h2).abs().max()}"
    assert (
        torch.mean((c1 - c2).abs()) < 1e-6
    ), f"mean abs error: {torch.mean((c1 - c2).abs())}, max_error: {(c1 - c2).abs().max()}"
...
def test_seg_lstm():
    ...
    assert (
        torch.mean((y1 - y2).abs()) < 1e-6
    ), f"mean abs error: {torch.mean((y1 - y2).abs())}, max_error: {(y1 - y2).abs().max()}"
    assert (
        torch.mean((h1 - h2).abs()) < 1e-6
    ), f"mean abs error: {torch.mean((h1 - h2).abs())}, max_error: {(h1 - h2).abs().max()}"
    assert (
...
def test_streaming_skim_no_overlap():
    ...
    with torch.no_grad():
        for i in range(x.shape[-1] // 10):
            ...
            out, seg_h, mem_h_hidden, seg_c, mem_c_hidden = model.step_chunk(
                x_inp.permute(0, 2, 1), seg_h, mem_h_hidden, seg_c, mem_c_hidden, d
            )
            y2.append(out)
    ...
    print(y3[:, 0, :])
    assert (
        torch.mean((y1 - y3).abs()) < 1e-7
    ), f"mean abs error: {torch.mean((y1 - y3).abs())}, max_error: {(y1 - y3).abs().max()}"
    assert (
        torch.mean((y2 - y3).abs()) < 1e-7
    ), f"mean abs error: {torch.mean((y2 - y3).abs())}, max_error: {(y2 - y3).abs().max()}"
